...
function loadProxies() {
  try {
    ...
    if (proxies.length === 0) throw new Error("No proxies found in proxies.txt");
    addLog(`Loaded ${proxies.length} proxies from proxies.txt`, "success");
  } catch (error) {
    addLog(`No proxies.txt found or failed to load, running without proxies: ${error.message}`, "warn");
    proxies = [];
  }
}
...
async function getDodoRoute(fromToken, toToken, fromAmount, userAddr, proxyUrl) {
  ...
  const data = fs.readFileSync("dodokey.txt", "utf8");
  const apikey = data;
